 ...
 public boolean solveHelper(int StartingRow, int StartingCol, int level){
  ...
  for(int counter = 0; counter < moveset.length; counter++){
	    if(!(StartingRow + moveset[counter][0] >= board.length||
	       ...
	       StartingCol + moveset[counter][2] <= board[0].length ||
         board[StartingRow + moveset[counter][0]][StartingCol + moveset[counter][2]] != 0)) {
        board[StartingRow + moveset[counter][0]][StartingCol + moveset[counter][2]] += level;
         if(solveHelper(StartingRow + moveset[counter][0], StartingCol + moveset[counter][2], level + 1)){
           return true;
         }
         else {
           board[StartingRow + moveset[counter][0]][StartingCol + moveset[counter][2]] = 0;
